...
public class MyParser implements IMiniCCParser {
	...
	public ASTStatement state() {
		...
		if(nextToken.type.equals("'{'")) {
			...
			return iterationstate();
		} else if(nextToken.type.equals("'do'")){
			return iterationstate();
	...
	//	'if' '(' exp ')' state 'else if' '(' exp ')' state 'else' '(' state ')'
	public ASTSelectionStatement selectstate() {
		...
		if(tknList.get(tokenIndex).type.equals("'else'")) {
			matchToken("'else'");
			if(!tknList.get(tokenIndex).type.equals("'if'")){
				...
				matchToken("'if'");
				matchToken("'('");
				LinkedList<ASTExpression> cond1 = new LinkedList<>(exp());
				matchToken("')'");
				ASTStatement then2 = state();
				ss.otherwise = then2;
				ss.children.add(then2);
				matchToken("'else'");
				ASTStatement then3 = state();
				ss.otherwise1 = then3;
				ss.children.add(then3);
			}
		}
		return ss;
	}
	...
	public ASTStatement iterationstate() {
		...
		if(nextToken.type.equals("'for'")){
			...
			ASTIterationStatement is = new ASTIterationStatement();
			LinkedList<ASTExpression> ie=null;
			LinkedList<ASTExpression> step;
			ASTStatement s = state();
			matchToken("'while'");
			matchToken("'('");
	...
	public ASTReturnStatement except_return(ASTReturnStatement rs){
		if(!tknList.get(tokenIndex).type.equals("';'")) {
	...
	public ArrayList<ASTExpression> exp(){
		...
		ASTExpression ea = exp_assign();
		ealist.add(ea);
		ArrayList<ASTExpression> ealist2 = except_exp_assign();
		ealist.addAll(ealist2);
		return ealist;
	}

	public ArrayList<ASTExpression> except_exp_assign(){
		ArrayList<ASTExpression> ealist2 = new ArrayList<ASTExpression>();
		nextToken = tknList.get(tokenIndex);
		if(nextToken.type.equals("','")) {
			matchToken("','");
			ealist2= exp();
	...
	//exp_shift
	//  exp_additive except_exp_additive
	...
	public ASTExpression exp_shift(){
		...
		if(tknList.get(tokenIndex).type.equals("'<<'") ||
				tknList.get(tokenIndex).type.equals("'>>'"))
		{
			ASTExpression es = new ASTBinaryExpression();
			es = except_exp_multiplicative(ea);
			return es;
		}
		else {
			return ea;
	...
	public ASTExpression except_exp_additive(ASTExpression ea) {
		ASTBinaryExpression eea = new ASTBinaryExpression();
		ASTToken op = getToken();
		...
		eea.op = op;
		eea.children.add(ea);
		eea.children.add(es2);
		eea.children.add(op);
		return eea;
	}

	//exp_additive
	//  exp_multiplicative except_exp_multiplicative

	//except_exp_multiplicative
	//  e
	//  '+' exp_additive
	//  '-' exp_additive
	public ASTExpression exp_additive(){
		...
		if(tknList.get(tokenIndex).type.equals("'+'") ||
				tknList.get(tokenIndex).type.equals("'-'"))
		{
			ASTExpression ea = new ASTBinaryExpression();
			ea = except_exp_multiplicative(em);
			return ea;
		}
		else {
			return em;
	...
	public ASTExpression except_exp_multiplicative(ASTExpression em) {
		ASTBinaryExpression eem = new ASTBinaryExpression();
		ASTToken op = getToken();
	...
	public ASTExpression exp_multiplicative(){
		...
		if(tknList.get(tokenIndex).type.equals("'*'") ||
				...
				tknList.get(tokenIndex).type.equals("'%'"))
		{
			ASTExpression em = new ASTBinaryExpression();
			em = except_exp_cast(ec);
			return em;
		}
		else {
			return ec;
	...
	public ASTExpression except_exp_cast(ASTExpression ec) {
		ASTBinaryExpression eec = new ASTBinaryExpression();
		ASTToken op = getToken();
