...
public class ExampleICBuilder implements ASTVisitor{
	...
	public void visit(ASTBinaryExpression binaryExpression) throws Exception {
		...
		if (op.equals("=")) {
			...
			if (binaryExpression.expr2 instanceof ASTIdentifier) {
				...
				ASTFunctionCall funcCall = (ASTFunctionCall)binaryExpression.expr2;
				visit(binaryExpression.expr2);
				String funcname = ((ASTIdentifier)funcCall.funcname).value;
				String functype = funcs.get_func_type(funcname);
				ControlLabel func_type = new ControlLabel(functype);
				ControlLabel func_name = new ControlLabel(funcname);
				opnd2 = func_name;
				opnd1 = func_type;
		...
		}else if (op.equals("+") || op.equals("-") || op.equals("*") || op.equals("/") || op.equals("%") || op.equals("<<")|| op.equals(">>")|| op.equals("<") || op.equals(">") ||
	...
	public void visit(ASTPostfixExpression postfixExpression) throws Exception {
		...
		quats.add(quat1);
		ControlLabel opnd1= new ControlLabel(((ASTIdentifier) postfixExpression.expr).value);
		Quat quat2 = new Quat(op, opnd1, opnd1, null);
		quats.add(quat2);
		res = tmp;
	...
	public void visit(ASTSelectionStatement selectionStat) throws Exception {
		Iterator ite = (selectionStat).cond.iterator();
		while(ite.hasNext()){
			ASTExpression expr = (ASTExpression) ite.next();
			visit(expr);
		}
	...
	public void visit(ASTUnaryExpression unaryExpression) throws Exception {
		...
		if(op.equals("++") ||
			...
			if(unaryExpression.expr instanceof ASTIdentifier) {
				ControlLabel opnd2 = new ControlLabel(((ASTIdentifier) unaryExpression.expr).value);
				Quat quat = new Quat(op, res, null, opnd2);
				quats.add(quat);
	...
	public void visit(ASTFunctionDefine functionDefine) throws Exception {
		...
		ControlLabel tmpscope = this.localscope;
		String specifier = "";
		for(ASTToken SpecifierNode : functionDefine.specifiers){
			specifier += SpecifierNode.value;
		}
		String name = functionDefine.declarator.getName();
		localscope = new ControlLabel(name);
		Quat quatfunctype = new Quat("Func_Type",functionDefine.declarator,functionDefine.specifiers.get(0),null);
		quats.add(quatfunctype);

		LinkedList params = new LinkedList();
		Iterator ite = ((ASTFunctionDeclarator)functionDefine.declarator).params.iterator();
		while(ite.hasNext()){
			ASTParamsDeclarator param = (ASTParamsDeclarator) ite.next();
			String param_spec = "";
			String param_name = param.declarator.getName();
			Iterator ite1 = param.specfiers.iterator();
			while(ite1.hasNext()){
				param_spec += ite1.next();
			}
			params.add(param_spec);
			this.localtable.addvar(param_name, param_spec);

			Quat quat_param = new Quat("param",param.declarator,param.specfiers.get(0),localscope);
			quats.add(quat_param);
		}
		funcs.addfunc(name, specifier, params);
		SymbolTable ftable = new SymbolTable();
		ftable = this.localtable;
		functable.put(name, ftable);
		this.localtable = ftable;
		functionDefine.scope = localtable;
//		functionDefine.declarator.scope = new SymbolTable();
//		functionDefine.declarator.scope.father = localtable;
